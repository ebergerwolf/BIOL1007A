...
test_plot <- ggplot() +
  geom_bar(data = testing_prop, aes(x=algebra, y=meanCorrect, 
                                   fill=direction, color=I("black"),size=I(0.3)),
  ...
  theme_base() +
  geom_errorbar(data=testing_prop, aes(x=algebra, ymax=meanCorrect+0.095, ymin=meanCorrect-.095),
                position=position_dodge2(reverse=T, padding=0.8, width=1), width=0.9, size=0.4)

## to-do:
# fix legend (change labels, switch points, move to top)
# fix yticks
# add x labels for each test: (1),(2),(3),(4)
# make individual bars skinnier
